...
#[test]
fn planner_output_requires_tagged_audience_shape() {
    for audience in [
        r#""global_writer""#,
        r#"{"kind":"unknown"}"#,
        ...
        let payload = format!(
            r#"{{"story_goal":"x","context_gaps":[{{"audience":{audience},"target_id":null,"query_text":"need","reason":"why"}}],"character_think_requests":[]}}"#
        );
        assert!(
            serde_json::from_str::<PlannerOutput>(&payload).is_err(),
            "accepted malformed audience: {audience}"
        );
    }
}
...
fn writer_planner_schema_requires_contract_fields_and_selector_exclusivity() {
    ...
    assert!(schema["properties"]["context_gaps"]["items"]["properties"]["audience"]["oneOf"].is_array());
    assert_eq!(
