...
fn null_optional_fields_default_to_empty() {
    let proposal: StoryProposal = serde_json::from_str(
        r#"{"story_text":"hello","events":null,"character_changes":null,"world_change":null,"memory_changes":null,"summary_delta":null}"#,
    )
    ...
    assert!(proposal.summary_delta.is_none());
}
...
fn world_change_null_add_facts_default_to_empty() {
    let proposal: StoryProposal = serde_json::from_str(
        r#"{"story_text":"hello","events":[],"character_changes":[],"world_change":{"add_facts":null},"memory_changes":[],"summary_delta":null}"#,
    )
...
fn nested_character_change_null_vectors_default_to_empty() {
    let proposal: StoryProposal = serde_json::from_str(
        r#"{"story_text":"hello","events":[],"character_changes":[{"character_id":"char-1","goal_updates":null,"health_delta":null,"affinity_deltas":null}],"world_change":{"add_facts":[]},"memory_changes":[],"summary_delta":null}"#,
    )
